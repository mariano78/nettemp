		...
		if ( $c >= "1") {
			if (is_numeric($val)) {
				...
				if ($val != 'range'){
					...
					$dbr->exec("UPDATE nt_setting SET value = value +1  WHERE option='refreshcount'") or die (date("Y-m-d H:i:s")." ERROR: Cannot insert count to table\n" ;logs(date("Y-m-d H:i:s"),'Error',$rom." - cicicicici"); );
