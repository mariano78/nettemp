...
foreach ($row_meteo as $a) {
	...
	if(($_SESSION["perms"] == 'adm') || (isset($_SESSION["user"]))) {
		...
		$sth = $db->prepare("SELECT * FROM sensors WHERE hide ='off' AND ch_group='$ch_g' AND type!='elec' AND type!='water' AND type!='gas' AND ch_group!='switch' AND ch_group!='relay' AND (jg!='on' OR jg is null) ORDER BY position ASC");
	} else {
		
		$sth = $db->prepare("SELECT * FROM sensors WHERE hide ='off' AND ch_group='$ch_g' AND type!='elec' AND type!='water' AND type!='gas' AND ch_group!='switch' AND ch_group!='relay' AND logon =='on' AND (jg!='on' OR jg is null) ORDER BY position ASC");
	}
	
	$gname = str_replace('_', ' ', $ch_g);
	$gname2 = '<a href="index.php?id=device&type=device&device_group='.$ch_g.'&device_menu=settings" title="Go to group settings" class="group-name" >'.str_replace("_", " ", $ch_g).'</a>';
...
?>
			...
			<?php if(!empty($a['mail']) && isset($_SESSION['user'])) { ?>
				...
				<input type="image" align="top" src="media/ico/message-icon.png" alt="Clear Notifications" />
				</form>
				<?php
				} else {echo $mail; }
				?>
