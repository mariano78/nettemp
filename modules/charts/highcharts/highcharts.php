...
if(($_SESSION["perms"] == 'adm') || (isset($_SESSION["user"]))) {
		...
		$dirb = "sqlite:dbf/nettemp.db";
		$db = new PDO($dirb) or die("cannot open database");
		$query = "select name,type FROM sensors WHERE type='$type' AND name='$single' AND logon ='on' AND position !='0'";
		foreach ($db->query($query) as $row) {
			$array[]=$row[0];
			$types[$row[0]]=$row[1];
			}
		...
		$dirb = "sqlite:dbf/nettemp.db";
		$db = new PDO($dirb) or die("cannot open database");
		$query = "select name,type FROM sensors WHERE ch_group='$group' AND charts='on' AND logon ='on' AND position !='0'";
		foreach ($db->query($query) as $row) {
			$array[]=$row[0];
			$types[$row[0]]=$row[1];
			}
		...
		$dirb = "sqlite:dbf/nettemp.db";
		$db = new PDO($dirb) or die("cannot open database");
		$query = "select name,type FROM sensors WHERE type='$type' AND charts='on' AND logon ='on'  AND position !='0'";
		foreach ($db->query($query) as $row) {
			$array[]=$row[0];
			$types[$row[0]]=$row[1];
			}
