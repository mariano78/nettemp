...
<tbody>
	...
	<?php
		...
		$rows = $dbs->query("SELECT time AS date,round(sum(value),3) AS sums from def WHERE strftime('%Y',time) IN ('$repyearselect') AND strftime('%m',time) IN ('*')  GROUP BY strftime('%m',time), strftime('%d',time)") or die('Something is wrong');
