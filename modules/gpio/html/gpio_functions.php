...
//time
    ...
    if ($timeon == "timeon")  {
		$db->exec("UPDATE gpio SET mode='time' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
...
//temp
    ...
    if ($tempon == "tempon")  {
		$db->exec("UPDATE gpio SET mode='temp' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
//humid
    ...
    if ($humidon == "humidon")  {
		$db->exec("UPDATE gpio SET mode='humid' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
...
//dayon
    ...
    if ($dayon == "dayon")  {
		$db->exec("UPDATE gpio SET mode='day' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
//weekon
    ...
    if ($weekon == "weekon")  {
		$db->exec("UPDATE gpio SET mode='week' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
...
//triggeron
	...
	//	$db = new PDO('sqlite:dbf/nettemp.db') or die("cannot open the database");
		$db->exec("UPDATE gpio SET mode='trigger' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
//kwh
    ...
    if ($kwhon == "kwhon")  {
		$db->exec("UPDATE gpio SET mode='kwh' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
    ...
    if ($readon == "readon")  {
		$db->exec("UPDATE gpio SET mode='read' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
    ...
    if ($diston == "diston")  {
		$db->exec("UPDATE gpio SET mode='dist' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
...
//simple
    ...
    if ($simpleon == "simpleon")  {
		$db->exec("UPDATE gpio SET mode='simple' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
...
//	$db = new PDO('sqlite:dbf/nettemp.db') or die("cannot open the database");
	$db->exec("UPDATE gpio SET mode='buzzer', status='' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
	$db = null;
	header("location: " . $_SERVER['REQUEST_URI']);
	exit();
    }
...
//triggerout
    ...
    if ($triggerout == "triggerout")  {
		$db->exec("UPDATE gpio SET mode='triggerout', status='' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
	exit();
    }
...
//call
    ...
    if ($call == "on")  {
		$db->exec("UPDATE gpio SET mode='call', status='' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
	exit();
    }
...
//functiononoff
    ...
    if ($control == "on")  {
		$db->exec("UPDATE gpio SET mode='control', status='' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
    ...
    if ($led == "on")  {
		$db->exec("UPDATE gpio SET mode='led', status='' WHERE gpio='$gpio_post' AND rom='$rom'") or die("exec error");
		$db = null;
		header("location: " . $_SERVER['REQUEST_URI']);
		exit();
    }
	...
	if ($gpiodel == "gpiodel")  {
		//maps settings
		$to_delete=$db->query("SELECT id FROM gpio WHERE gpio='$gpio_post' AND rom='$rom'");
